...
input_dir  <- file.path(parent_path, "og_zipped")
output_dir <- file.path(parent_path, "og_unzipped")

# 2. Create the main output directory if needed
...
message("Processing ", length(zip_files), " files...")

# 4. Loop through each file, create a dedicated folder, and unzip
invisible(lapply(zip_files, function(f) {
  
  # A. Get the name of the file without the ".zip" extension
  # e.g., "my_shapefile.zip" becomes "my_shapefile"
  folder_name <- tools::file_path_sans_ext(basename(f))
  
  # B. Create a specific path for this sub-folder
  sub_dir <- file.path(output_dir, folder_name)
  
  # C. Create the sub-folder (showWarnings=FALSE suppresses warnings if it already exists)
  dir.create(sub_dir, showWarnings = FALSE)
  
  # D. Unzip strictly into this new sub-folder
