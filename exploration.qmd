...
county <- read.csv("data/prep/counties/2020_county_presidential.csv")
...
ggplot(county_sf) +
  ...
  theme_void() +
  labs(
    title = "US Counties – 2020 Presidential Vote Share",
...
2020 -- WITH THE NEW DATA (newer version, same data) let's see what we get
```{r}
...
# ==========================================
# 3. Random Forest (Modified Formula)
# ==========================================

# NOTE: We subtract ID and density from the formula so the model 
# doesn't train on them, but they stay in the dataframe.
rf_model <- ranger(
  TRUMPSHARE ~ . - precinct_id - vote_density, 
  data       = train_df,
  num.trees  = 500,
  mtry       = 8,
  importance = "impurity",
...
# ==========================================
# 4. Predict & Join Results
# ==========================================

# Make predictions on test set
preds <- predict(rf_model, data = test_df)$predictions

# Create a results dataframe with original metadata
results <- test_df %>%
  mutate(
    Predicted_Share = preds,
...
tss  <- sum((results$TRUMPSHARE - mean(results$TRUMPSHARE))^2)
r2   <- 1 - rss / tss
rmse <- sqrt(mean(results$Error^2))

cat("\nTest Set R²:", round(r2, 3))
cat("\nTest Set RMSE:", round(rmse, 4), "\n")

# ==========================================
# 5. Plot 1: Prediction Error vs. Density
# ==========================================

# We use log10 scale for density because it spans orders of magnitude
plot1 <- ggplot(results, aes(x = vote_density, y = Error)) +
  ...
  labs(
    ...
    subtitle = "Positive Error = Model Overpredicted Trump Share",
...
# 1. Bin density into 10 deciles
decile_stats <- results %>%
  ...
  summarize(
    ...
    # R2 calculation per group
    R2 = 1 - (RSS / TSS)
  )

# 2. Plot performance by decile
plot2 <- ggplot(decile_stats, aes(x = factor(density_decile), y = R2)) +
  ...
  labs(
    ...
    subtitle = "Does the model understand rural areas better than urban ones?",
    x = "Density Decile (1 = Most Rural, 10 = Most Urban)",
    y = "R-squared"
  ) +
  theme_minimal()
...
# 1️⃣ Ensure embeddings are numeric and select columns
df2_clean <- df2 %>%
...
# 3️⃣ Create DMatrix objects (xgboost's optimized data structure)
dtrain <- xgb.DMatrix(data = X_train, label = y_train)
dtest  <- xgb.DMatrix(data = X_test,  label = y_test)

watchlist <- list(train = dtrain, eval = dtest)

# 4️⃣ Set XGBoost parameters
params <- list(
  booster          = "gbtree",
...
# ==========================================================
# 🔟 Fit final model on full training data, evaluate on test
# ==========================================================
...
# ==========================================================
# 🔟 Fit final model on full training data, eval on test
# ==========================================================
