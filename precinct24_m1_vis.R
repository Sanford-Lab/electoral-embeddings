...
cat("\n==========================================")
cat("\n2024 OVERALL PRECINCT CV RESULTS")
cat("\n==========================================")
...
plot1 <- ggplot(full_results, aes(x = vote_density, y = Error)) +
  ...
  labs(
    title = "2024 Precinct Prediction Error vs. Voter Density (5-Fold CV)",
...
plot2 <- ggplot(decile_stats, aes(x = factor(density_decile), y = R2)) +
  ...
  labs(
    title = "2024 Model Performance by Density Decile (Precincts)",
...
plot3 <- ggplot(full_results, aes(x = TRUMPSHARE, y = Predicted_Share)) +
  ...
  labs(
    title = "2024 Actual vs. Predicted Trump Share (Precinct Level)",
